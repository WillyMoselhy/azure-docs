...
- Setting the property `treatFailureAsDeploymentFailure` to true will fail the deployment and ProvisionState will be marked as Failed if the script fails. IF set to False (Default), ProvisioningState would only reflect whether the run command was run or not by the extensions platform, it would not indicate whether script failed in case of script failures. This is only available under API version 2023-03-01 and later.
